...
for dir in available_dirs:
    ...
    ax.hlines(plotting_df_rsa.groupby("ROI", sort=False).agg("first")["LNC"].reindex(rois), x-0.35, x+0.35, color='k', ls='dotted', label="noise ceiling")
    ax.hlines(plotting_df_rsa.groupby("ROI", sort=False).agg("first")["UNC"].reindex(rois), x-0.35, x+0.35, color='k', ls='dotted')
    ax.tick_params(axis='x', labelrotation=45, size=12, labelsize=12)
    ax.tick_params(axis='y', labelsize=12)
    ax.set_ylabel("R (pearson)", fontsize=16)
    ax.set_xlabel("ROI", fontsize=16)
    ...
    ax.hlines(plotting_pooled.groupby("ROI", sort=False)["LNC"].agg("first"), x-0.35, x+0.35, color='k', ls='dotted', label="noise ceiling")
    ax.hlines(plotting_pooled.groupby("ROI", sort=False)["UNC"].agg("first"), x-0.35, x+0.35, color='k', ls='dotted')
    ax.tick_params(axis='x', labelrotation=45, size=12, labelsize=12)
    ax.tick_params(axis='y', labelsize=12)
    ax.set_ylabel("R (pearson)", fontsize=16)
    ax.set_xlabel("ROI", fontsize=16)
...
for roi in rsa_comp["ROI"].unique():
    ...
    ax.hlines(roi_df_expl.groupby("Model", sort=False).agg("first")["LNC"], x-0.35, x+0.35, color='k', ls='dotted', label="noise ceiling")
    ax.hlines(roi_df_expl.groupby("Model", sort=False).agg("first")["UNC"], x-0.35, x+0.35, color='k', ls='dotted')
